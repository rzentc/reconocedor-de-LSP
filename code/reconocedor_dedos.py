...
def mark_fingers(frame_in,hull,pt,radius):
    ...
    count_text="DEDOS:"+str(finger_count)
...
def find_gesture(frame_in,finger,palm):
    ...
    gesture_text="GESTO:"+str(gesture_found)
...
# Camera
camera = cv2.VideoCapture('http://192.168.8.103:4747/mjpegfeed?1280x720')
...
while(1):
    ...
    #cvtColor(frame,frame,Imgproc.COLOR_BGRA2BGR );
    ...
    if (not (capture_done and bg_captured)):
        if(not bg_captured):
            cv2.putText(frame,"",(int(0.05*frame.shape[1]),int(0.97*frame.shape[0])),cv2.FONT_HERSHEY_SIMPLEX,0.5,(0,255,255),1,8)
        else:
            cv2.putText(frame,"",(int(0.08*frame.shape[1]),int(0.97*frame.shape[0])),cv2.FONT_HERSHEY_SIMPLEX,0.5,(0,255,255),1,8)
        first_iteration=True
        finger_ct_history=[0,0]
